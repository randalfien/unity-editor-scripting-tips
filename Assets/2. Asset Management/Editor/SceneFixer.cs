...
public class SceneFixer : MonoBehaviour
{
    ...
    private static void FixSceneRefs()
    {
        ...
        for (var i = 0; i < sceneGuids.Length; i++)
        {
            ...
            {
                var sceneRefs = rootObject.GetComponentsInChildren<TestBehaviour>();
                
                foreach (var testBehaviour in sceneRefs)
                {
                    if (testBehaviour.Variable1 % 2 == 1)
                    {
                        testBehaviour.Variable1 += 1;
                        change = true;
                    }
                }
            }
    ...
    /*
     This is a good way to iterate over all scenes, call like this: foreach (Scene scene in GetSavedScenes()) { ... }
